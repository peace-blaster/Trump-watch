...
The terminal view includes a compact ASCII building banner, percentage bars for
the latest published responses, and an aligned table when requesting history.
Retrieval status, freshness, sample base, and source links appear alongside the
data. Color is automatic on terminals; redirected output stays plain text.
Set `NO_COLOR=1` to disable color. `--json` remains machine-readable.
